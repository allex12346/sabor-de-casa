...
- **Diferenciais Implementados:**
  ...
  - Painel simples de gerenciamento e cadastro de produtos no banco (`/admin`).
...
## 5. Publicação e Hospedagem Externa

A aplicação foi projetada para deploy simples em serviços de nuvem:

### Opção 1: Vercel / Railway / Render (Full-Stack)
- **Front-end + Back-end:** Pode ser executado em container Node.js no **Railway**, **Render** ou **Fly.io** executando `pnpm build` e `pnpm start`.
- **Banco de Dados em Nuvem:** Conecte o `DATABASE_URL` a qualquer serviço gerenciado na nuvem como:
  - **Neon** (Postgres) ou **PlanetScale** / **TiDB Cloud** / **Aiven** / **Supabase** / **Railway MySQL**.

### Opção 2: Separação Front (Vercel/Netlify) + Back (Render/Railway)
- O build do front-end (`client`) gera arquivos estáticos sob `dist/public`, que podem ser servidos diretamente em **Vercel** ou **Netlify**, apontando as chamadas de API para a URL do servidor Node.js/Express.
